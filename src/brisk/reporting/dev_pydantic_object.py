...
class DevExperimentGroupCard(BaseModel):
    group_name: str
...
class DevReportHome(BaseModel):
    title: str
    brisk_version: str
    timestamp: str
    tables: List[TableData] = Field(
        default_factory=list, description="List of tables to render"
    )
    experiment_group_cards: Dict[str, DevExperimentGroupCard] = Field(
        default_factory=dict,
...
if __name__ == "__main__":
    ...
    experiment_group_1 = DevExperimentGroupCard(
    ...
    experiment_group_2 = DevExperimentGroupCard(
    ...
    experiment_group_3 = DevExperimentGroupCard(
    ...
    experiment_group_4 = DevExperimentGroupCard(
    ...
    env = Environment(loader=FileSystemLoader(searchpath="./src/brisk/reporting/templates"))
    template = env.get_template('report.html')
    html_output = template.render(report=report_data.model_dump())

    with open('experiment_report.html', 'w', encoding='utf-8') as f:
        f.write(html_output)
        print("File saved to experiment_report.html")
